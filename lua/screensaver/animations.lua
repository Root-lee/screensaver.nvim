	...
	update = function(grid, state)
		...
		local function cell_empty(g, x, y)
			if
				...
				and y > 0
				and y <= width
				and (g[x][y].char == " " or g[x][y].char == utils.nbsp)
			then
				return true
			end
			return false
...
animations.game_of_life = {
	fps = 10,
	init = function(grid)
		return { over = 4, under = 1, respawn = 3 }
	end,
	update = function(grid, state)
	...
	update = function(grid, state)
		...
		for _, star in ipairs(state.stars) do
			...
			if sx >= 1 and sx <= state.width and sy >= 1 and sy <= state.height then
				local char = "."
				if star.z < state.width / 4 then
					char = "@"
				elseif star.z < state.width / 2 then
					char = "*"
				elseif star.z < state.width / 1.5 then
					char = "+"
				end
...
animations.pipes = {
	...
	update = function(grid, state)
		...
		for _, pipe in ipairs(state.pipes) do
			...
			if pipe.x > state.width then
				pipe.x = 1
			end
			if pipe.y < 1 then
				pipe.y = state.height
			end
			if pipe.y > state.height then
				pipe.y = 1
			end
			...
			if old_dx ~= pipe.dx or old_dy ~= pipe.dy then
				if (old_dy == 1 and pipe.dx == 1) or (old_dx == -1 and pipe.dy == -1) then
					char = "┌"
				elseif (old_dy == 1 and pipe.dx == -1) or (old_dx == 1 and pipe.dy == -1) then
					char = "┐"
				elseif (old_dy == -1 and pipe.dx == 1) or (old_dx == -1 and pipe.dy == 1) then
					char = "└"
				elseif (old_dy == -1 and pipe.dx == -1) or (old_dx == 1 and pipe.dy == 1) then
		...
		for r = 1, state.height - 1 do
			for c = 1, state.width do
				...
				if state.heat[r + 1] then
					...
					count = count + 1
					if c > 1 then
						sum = sum + state.heat[r + 1][c - 1]
						count = count + 1
					end
					if c < state.width then
						sum = sum + state.heat[r + 1][c + 1]
						count = count + 1
					end
				end
...
animations.zoo = {
	...
	init = function(grid)
		...
		local num_animals = math.max(5, math.floor((width * height) / 400))
		...
		for i = 1, num_animals do
			table.insert(animals, {
				...
				char = chars[math.random(1, #chars)],
			})
		end
		return { animals = animals, width = width, height = height }
	end,
	update = function(grid, state)
		if not state.bg then
			state.bg = vim.deepcopy(grid)
		...
		for _, a in ipairs(state.animals) do
			if math.random() < 0.05 then
				a.dx = (math.random() - 0.5) * 2
				a.dy = (math.random() - 0.5) * 1
			end

			a.x = a.x + a.dx
			a.y = a.y + a.dy

			if a.x > state.width + 2 then
				a.x = -2
			end
			if a.x < -2 then
				a.x = state.width + 2
			end
			if a.y > state.height + 2 then
				a.y = -2
			end
			if a.y < -2 then
				a.y = state.height + 2
			...
			local draw_x = math.floor(a.x)
			local draw_y = math.floor(a.y)

			if draw_y >= 1 and draw_y <= state.height and draw_x >= 1 and draw_x <= state.width then
				put_char(grid, draw_x, draw_y, a.char, "Screensaver")
			end
		end
		return true
	end,
